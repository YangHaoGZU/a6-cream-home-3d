...
export default function HomeTour(){
 ...
 useEffect(()=>{api.current?.setOptions({mode,ceiling,cutaway,dimensions,furniture});},[mode,ceiling,cutaway,dimensions,furniture,ready]);
 ...
 return <main className="tour-shell">
  <header className="topbar"><div className="brand"><span className="monogram">A<span>6</span></span><div><h1>奶油风 · 全屋漫游</h1><p>190.65㎡ 户型 / 家具 · 家电 · 厨卫</p></div></div>
  ...
  <section className="viewport" aria-label="户型漫游工作区"><div className="three-host" ref={host}/>
   {!ready&&!error&&<div className="loading"><span/><b>正在构建你的家</b><p>载入家具、厨卫、瓷砖与光线…</p></div>}
   ...
   <aside className="material-card"><div className="card-eyebrow">{mode==='walk'?'当前空间':'全屋方案'}<span>02</span></div><h2>{mode==='walk'?current.name:'暖白 · 浅米石纹'}</h2><p className="room-note">{mode==='walk'?current.note:'奶油色软包、浅石色柜体与细节五金，延续全屋瓷砖。'}</p>
   ...
   <aside className={'mini-map '+(!mapOpen?'collapsed':'')}><button className="map-title" onClick={()=>setMapOpen(!mapOpen)}><span>户型导航 <small>点击空间进入</small></span>{mapOpen?<X size={14}/>:<Scan size={17}/>}</button>{mapOpen&&<><svg viewBox="-1 -1.3 18.4 20.6" role="img" aria-label="按户型比例绘制的空间导航，上北下南">
    ...
    <path d="M7.2 13.05H6.35M6.35 13.05A.9 .9 0 0 0 7.2 13.95" fill="none" stroke="#8b785d" strokeWidth=".055" pointerEvents="none"/>
